...
export function PostCard({ post }: PostCardProps) {
  return (
    <article className="overflow-hidden rounded-[26px] bg-[#f8f8f8] shadow-[0_12px_30px_rgba(15,23,42,0.08)]">
      ...
      <div className="p-4">
        <h3 className="text-[20px] font-bold leading-tight text-slate-700">{post.title}</h3>
        ...
        <p className="mt-3 text-[18px] font-medium italic text-[#f2483a]">{post.availableLabel}</p>

        <p className="mt-4 text-right text-[16px] text-slate-400">
          từ <span className="text-[24px] font-extrabold text-[#25c3c8]">{post.priceLabel}</span>
        </p>
      </div>
    </article>
  );
